...
export class ObligacionComponent implements OnInit {
    ...
    openEdit(item: ObligacionResponse) {
        this.service.find({ id: item.id }).subscribe({
            next: (resultResponse) => {
                this.record = {
                    ...
                    fechaRegistro: resultResponse.fechaRegistro,
                    estadoId: resultResponse.estado?.id,
                    tipoId: resultResponse.tipo?.id,
                    pedidoId: resultResponse.pedido?.id,
                    monto: resultResponse.monto
                };
            },
            error: (err) => {
                Swal.close();
